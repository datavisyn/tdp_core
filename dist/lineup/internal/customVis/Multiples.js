...
export function Multiples(props) {
    ...
    let updateSelectedCatCols = (s) => setSelectedCatCols(s);
    let updateSelectedNumCols = (s) => setSelectedNumCols(s);
    useEffect(() => {
    ...
    let shapeScale = useMemo(() => {
        return props.shape ?
            scale.ordinal().domain(d3.set(props.shape.vals.map(v => v.val)).values()).range(["circle", "square", "triangle", "star"])
            : null;
    }, [props.shape]);
    ...
    let traces = currPlot.createTrace(props, selectedCatCols, selectedNumCols, shapeScale, colorScale, opacityScale, bubbleScale);
    let layout = {
        showlegend: true,
        legend: {
            itemclick: false,
