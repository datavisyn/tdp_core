...
export function ColorSelect({ callback, numTypeCallback = () => null, currentNumType = null, columns, currentSelected, }) {
    return (React.createElement(React.Fragment, null,
        ...
        React.createElement(Select, { isClearable: true, onChange: (e) => callback(e), name: 'colorSelect', formatOptionLabel: formatOptionLabel, getOptionLabel: (option) => option.name, getOptionValue: (option) => option.id, options: (columns.map((c) => c.info)), value: currentSelected ? currentSelected : [] }),
        currentNumType && currentSelected && getCol(columns, currentSelected).type === EColumnTypes.NUMERICAL
            ? React.createElement(NumericalColorButtons, { callback: numTypeCallback, currentSelected: currentNumType })
            : null));
}
