...
import d3 from 'd3';
import { EColumnTypes, ENumericalColorScaleType, ESupportedPlotlyVis, } from '../interfaces';
...
export function createScatterTraces(columns, selected, config, scales, shapes) {
    ...
    const shapeScale = config.shape
        ? d3.scale
            .ordinal()
            .domain([...new Set(getCol(columns, config.shape).values.map((v) => v.val))])
            .range(shapes)
        : null;
    let min = 0;
    let max = 0;
            ...
            data: {
                ...
                marker: {
                    ...
                    color: getCol(columns, config.color)
                        ? getCol(columns, config.color).values.map((v) => selected[v.id] ? '#E29609' : getCol(columns, config.color).type === EColumnTypes.NUMERICAL ? numericalColorScale(v.val) : scales.color(v.val))
                ...
                plots.push({
                    data: {
                        ...
                        marker: {
                            ...
                            symbol: getCol(columns, config.shape) ? getCol(columns, config.shape).values.map((v) => shapeScale(v.val)) : 'circle',
                            color: getCol(columns, config.color)
                                ? getCol(columns, config.color).values.map((v) => selected[v.id]
                                    ? '#E29609'
                ...
                transforms: [
                    {
                        ...
                        styles: [
                            ...[...new Set(getCol(columns, config.color).values.map((v) => v.val))].map((c) => {
                ...
                transforms: [
                    {
                        ...
                        styles: [
                            ...[...new Set(getCol(columns, config.shape).values.map((v) => v.val))].map((c) => {
                                return { target: c, value: { name: c } };
                            }),
                        ],
                    },
