    ...
    constructor(document, options, ratio, child1, child2) {
        ...
        if (!this.options.fixed) {
            this.node.addEventListener('mousedown', (evt) => {
                if (this.isSeparator(evt.target)) {
                    // dragging
                    const index = Math.floor(Array.from(this.node.children).indexOf(evt.target) / 2);
        ...
        if (this.length > 1) {
            ...
            if (!this.options.fixed) {
                separator.firstElementChild.addEventListener('click', (evt) => {
                    evt.preventDefault();
                    evt.stopPropagation();
                    this.squeeze(separator, 'left');
                });
                separator.lastElementChild.addEventListener('click', (evt) => {
                    evt.preventDefault();
                    evt.stopPropagation();
                    this.squeeze(separator, 'right');
                });
            }
            else {
