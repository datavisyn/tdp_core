...
export class StoreUtils {
    static editDialog(namedSet, entitiesTitle, result) {
      ...
      <div class="mb-3">
        ...
        <input type="text" class="form-control" name="name" id="namedset_name" data-testid="${dialog.form.id}-editDialog-name-control" placeholder="${I18nextManager.getInstance().i18n.t('tdp:core.editDialog.name')}" required="required" ${namedSet ? `value="${namedSet.name}"` : ''}>
      </div>
      <div class="mb-3">
        <label class="form-label" for="namedset_description">${I18nextManager.getInstance().i18n.t('tdp:core.editDialog.description')}</label>
        <textarea class="form-control" name="description" id="namedset_description" rows="5" data-testid="${dialog.form.id}-editDialog-description-textarea" placeholder="${I18nextManager.getInstance().i18n.t('tdp:core.editDialog.description')}">${namedSet ? namedSet.description : ''}</textarea>
      </div>
    `;
