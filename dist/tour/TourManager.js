...
export class TourManager {
    constructor(context) {
        ...
        this.tourContext = {
            ...
            hide: (finished) => this.hideTour(finished),
    ...
    <div class="tdp-tour-step-navigation">
      ...
      <div class="btn-group" role="group">
        <button type="button" data-switch="--" class="btn-sm btn btn-secondary"><i class="fas fa-fast-backward"></i> ${I18nextManager.getInstance().i18n.t('tdp:core.TourManager.restartButton')}</button>
        <button type="button" data-switch="-" class="btn-sm btn btn-secondary"><i class="fas fa-step-backward"></i> ${I18nextManager.getInstance().i18n.t('tdp:core.TourManager.backButton')}</button>
        <button type="button" data-switch="0" class="btn-sm btn btn-secondary"><i class="fas fa-stop"></i> ${I18nextManager.getInstance().i18n.t('tdp:core.TourManager.cancelButton')}</button>
        <button type="button" data-switch="+" class="btn-sm btn btn-secondary"><i class="fas fa-step-forward"></i>${I18nextManager.getInstance().i18n.t('tdp:core.TourManager.nextButton')}</button>
      </div>
    </div>
    `;
        Array.from(this.step.querySelectorAll('.btn-group button')).forEach((button) => {
            button.onclick = (evt) => {
                evt.preventDefault();
                evt.stopPropagation();
                if (!this.activeTour) {
    ...
    <div class="modal-dialog" role="document">
        ...
        Array.from(this.chooser.querySelectorAll('.modal-body a')).forEach((a) => {
            a.onclick = (evt) => {
                evt.preventDefault();
    ...
    selectHighlight(selector) {
        ...
        const all = [].concat(...selector.map((d) => Array.from(this.step.ownerDocument.querySelectorAll(d))));
        if (all.length === 0) {
            return null;
        }
        if (all.length === 1) {
...
//# sourceMappingURL=TourManager.js.map