...
export function Multiples(props: MultiplesProps) {
    ...
    const shapeScale = useMemo(() => {
        return props.shape ?
            scale.ordinal<string>().domain(d3.set(props.shape.vals.map((v) => v.val)).values()).range(['circle', 'square', 'triangle-up', 'star'])
            : null;
    }, [props.shape]);
    ...
    const traces: MultipleDataTraces = currPlot.createTrace(props, selectedCatCols, selectedNumCols, shapeScale, colorScale, opacityScale, bubbleScale);
            ...
            <div className="d-flex justify-content-center align-items-center flex-grow-1">
                {traces.plots.length > 0 ?
                    (<Plot
                        ...
                        onSelected={(d) => d ? props.selectedCallback(d.points.map((d) => (d as any).id)) : props.selectedCallback([])}
