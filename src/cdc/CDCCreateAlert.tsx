...
import {accordionItem, DEFAULTALERTDATA, DEFAULTFILTER} from ".";
...
export function CDCCreateAlert({alertData, setAlertData, filterSelection, filter, setFilter, filterComponents, alertList, setAlertList, setCreationMode, setSelectedAlert}: ICDCCreateAlert) {
  const generalInformation =
    (<>
      ...
      <div className="mb-3">
        ...
        <input type="text" className="form-control" disabled={true} value={alertData.cdc_id} onChange={(e) => setAlertData({...alertData, cdc_id: e.target.value})} />
      </div>
  ...
  return (<>
    <div className="d-flex w-100 justify-content-between mb-1">
      ...
      <small>
        ...
        <button title="Discard changes" className="btn btn-secondary ms-1" onClick={() => setCreationMode(false)}><i className="fas fa-ban"></i></button>
      </small>
    </div>
