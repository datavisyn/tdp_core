      ...
      : <>
        {selectedAlert.latest_diff || selectedAlert.confirmed_data ? (<>
          ...
          {selectedAlert.latest_diff ? <div className="p-1">
            {confirmError ? <ErrorMessage error={new Error(`While confirming an error occured: ${confirmError}`)} /> : null}
            <div className="d-md-flex justify-content-md-end">
              <button title="Confirm changes" className="btn btn-primary" onClick={() => doConfirm()}>Confirm</button>
            </div>
          </div> : null}
        </>) : <p className="m-3">No new data available</p>}
      </>}
  </>);
}
