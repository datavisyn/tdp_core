...
class StateRepr {
  ...
  static render($elem: d3v3.Selection<StateRepr>) {
    $elem
...
export class LayoutedProvVis extends AVisInstance implements IVisInstance {
  ...
  private onStateClick(d: StateRepr) {
    ...
    this.data.selectState(d.s, SelectionUtils.toSelectOperation(<MouseEvent>d3v3.event));
  ...
  update() {
    ...
    const $statesEnter = $states
      ...
      .on('dragenter', function () {
        if (DnDUtils.getInstance().hasDnDType(<DragEvent>d3v3.event, 'application/phovea-prov-state')) {
