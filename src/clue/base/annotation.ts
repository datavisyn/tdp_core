...
import { SelectionUtils } from 'visyn_core';
...
class AnchorWatcher {
  ...
  private watch() {
    this.intervall = window.setInterval(this.check.bind(this), AnchorWatcher.UPDATE_INTERVALL);
  }
...
export class Renderer {
  ...
  render(state: SlideNode, withTransition = true, waitBetweenTakeDown = false) {
    ...
    // create full chain
    this.prev = this.prev.then(async () => {
      ...
      await takedown;
      await BaseUtils.resolveIn(waitBetweenTakeDown ? 1000 : 0);
      let next = null;
      if (state.isTextOnly) {
        // no state jump
        ...
        // jump to next state
        next = this.graph.jumpTo(
          state.state,
          state.transition <= 0 || !withTransition ? StoryTransition.MIN_TRANSITION : state.transition * StoryTransition.FACTOR,
        );
      }
      await next;
  ...
  private renderAnchors(bounds: { x: number; y: number; w: number; h: number }) {
    ...
    anchorElements.forEach((a) => {
      ...
      if (b.w * b.h < 50 * 50) {
        // area to small for higher details
        anchors.push(new Anchor(a, EAnchorDirection.CENTER));
      } else {
        anchors.push.apply(
          anchors,
          Object.keys(string2anchor).map((s) => new Anchor(a, string2anchor[s])),
        );
      }
    });
  ...
  private renderAnnotationsImpl(state: SlideNode) {
    ...
    function updateSize(d: IFrameStateAnnotation) {
      ...
      if (p) {
        ...
        const { pos } = Anchor.fromString((<any>d.pos).anchor);
  ...
  hideOld() {
    return new Promise<void>((resolve) => {
      this.anchorWatcher.clear();
      const $div = this.$main
        .classed('hide-all-non-annotations', false)
        .selectAll('div.annotation, div.text-overlay, div.add-text-annotation, div.subtitle-annotation');
