  ...
  static editDialog(namedSet: IStoredNamedSet, entitiesTitle: string, result: (name: string, description: string, sec: Partial<ISecureItem>) => void) {
    ...
    dialog.form.innerHTML = `
      ...
      <div class="mb-3">
        ...
        <input type="text" data-testid="name-input" class="form-control" name="name" id="namedset_name" placeholder="${I18nextManager.getInstance().i18n.t(
          'tdp:core.editDialog.name',
        )}" required="required" ${namedSet ? `value="${namedSet.name}"` : ''}>
      </div>
      <div class="mb-3">
        <label class="form-label" for="namedset_description">${I18nextManager.getInstance().i18n.t('tdp:core.editDialog.description')}</label>
        <textarea class="form-control" data-testid="description-input" name="description" id="namedset_description" rows="5" placeholder="${I18nextManager.getInstance().i18n.t(
          'tdp:core.editDialog.description',
        )}">${namedSet ? namedSet.description : ''}</textarea>
      </div>
    `;
