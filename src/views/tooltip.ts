...
export class TooltipUtils {
  ...
  private static showTooltipAt(tooltip: HTMLElement, reference: HTMLElement | { x: number; y: number }, simpleTooltip = true) {
    const p: Popper = (<any>tooltip).__popper;
    if (p) {
      p.destroy();
    }
    tooltip.style.display = 'block';
    (<any>tooltip).__popper = new Popper(
      'x' in <any>reference && 'y' in <any>reference
        ? {
            clientHeight: 1,
            clientWidth: 1,
  ...
  static showTooltip(html: string | HTMLElement | null, reference: HTMLElement | { x: number; y: number }, simpleTooltip = true) {
    if (!html) {
   ...
   */
  static popOver(html: string | HTMLElement | (() => string | HTMLElement), reference: HTMLElement, coords?: { x: number; y: number }, simpleTooltip = true) {
   ...
   */
  static generateTooltip<T>(contentGenerator: (items: T[]) => string | HTMLElement, simpleTooltip = true) {
    let tooltip: HTMLElement;
