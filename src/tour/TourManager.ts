...
export class TourManager {
  ...
  private showStep(stepNumber: number, step: IStep) {
    ...
    if (focus) {
      ...
      const scrollOffsetX = window.scrollX;
      const scrollOffsetY = window.scrollY;
      this.stepCount.style.transform = `translate(${base.left + scrollOffsetX + (step?.iconPlacementOffset?.x || 0)}px, ${base.top + scrollOffsetY + (step?.iconPlacementOffset?.y || 0)}px)`;
    } else {
