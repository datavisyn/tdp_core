...
class StateRepr {
  ...
  static toRepr(graph: ProvenanceGraph, highlight: any, options: any = {}) {
    //assign doi
    const lookup: any = {};
...
export class LayoutedProvVis extends AVisInstance implements IVisInstance {
  private $node: d3.Selection<any>;
  private trigger = BaseUtils.bind(this.update, this);
  private triggerStoryHighlight = BaseUtils.bind(this.updateStoryHighlight, this);
  ...
  private build($parent: d3.Selection<any>) {
    ...
    $p.html(`
      ...
      <div>
        ...
        <div class="btn-group" role="group">
          ...
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_data" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.dataActions')}" data-testid="${testId}-filter-dataActions">
            <i class="fas fa-database"></i>
          </label>
          <input type="checkbox" class="btn-check" autocomplete="off" id="btncheck_visual" name="category" value="visual">
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_visual" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.visualActions')}" data-testid="${testId}-filter-visualActions">
            <i class="fas fa-chart-bar"></i>
          </label>
          <input type="checkbox" class="btn-check" autocomplete="off" id="btncheck_selection" name="category" value="selection">
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_selection" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.selectionActions')}" data-testid="${testId}-filter-selectionActions">
            <i class="fas fa-pen-square"></i>
          </label>
          <input type="checkbox" class="btn-check" autocomplete="off" id="btncheck_layout" name="category" value="layout">
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_layout" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.layoutActions')}" data-testid="${testId}-filter-layoutActions">
            <i class="fas fa-desktop"></i>
          </label>
          <input type="checkbox" class="btn-check" autocomplete="off" id="btncheck_logic" name="category" value="logic">
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_logic" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.logicActions')}" data-testid="${testId}-filter-logicActions">
        ...
        <div class="btn-group" role="group">
          ...
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_create" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.createActions')}" data-testid="${testId}-actions-createActions">
             <i class="fas fa-plus"></i>
          </label>
          <input type="checkbox" class="btn-check" autocomplete="off" id="btncheck_update" name="operation" value="update">
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_update" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.updateActions')}" data-testid="${testId}-actions-updateActions">
             <i class="fas fa-sync"></i>
          </label>
          <input type="checkbox" class="btn-check" autocomplete="off" id="btncheck_remove" name="operation" value="remove">
          <label class="btn btn-outline-secondary btn-sm" for="btncheck_update" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.removeActions')}" data-testid="${testId}-actions-removeActions">
             <i class="fas fa-times"></i>
          </label>
        </div>

            <div class="dropdown">
              <button class="btn btn-outline-secondary dropdown-toggle btn-sm" type="button" data-bs-toggle="dropdown" data-testid="${testId}-filter-dropdown-tags-btn" aria-haspopup="true" aria-expanded="false">
              ...
              <div class="dropdown-menu dropdown-menu-end" data-bs-popper="static" aria-labelledby="dropdownMenuButton">
                <form class="px-1" onsubmit="return false;">
                  <div class="input-group input-group-sm">
                        ...
                        <input name="tags" type="text" class="form-control form-control-sm" data-testid="${testId}-filter-dropdown-tags" placeholder="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.tags')}" aria-describedby="provenance-filter-tags">
                  </div>
                </form>
              </div>
            </div>
      ...
      <input type="checkbox" class="btn-check" autocomplete="off" id="btncheck_vertical_1" name="category" value="data">
        <label class="btn btn-outline-secondary btn-sm mb-0" for="btncheck_vertical_1" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.dataActions')}" data-testid="${testId}-btn-vertical-dataActions">
        ...
        <label class="btn btn-outline-secondary btn-sm mb-0" for="btncheck_vertical_2" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.visualActions')}" data-testid="${testId}-btn-vertical-visualActions">
        ...
        <label class="btn btn-outline-secondary btn-sm mb-0" for="btncheck_vertical_3" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.selectionActions')}" data-testid="${testId}-btn-vertical-selectionActions">
        ...
        <label class="btn btn-outline-secondary btn-sm mb-0" for="btncheck_vertical_4" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.layoutActions')}" data-testid="${testId}-btn-vertical-layoutActions">
        ...
        <label class="btn btn-outline-secondary btn-sm mb-0" for="btncheck_vertical_5" title="${I18nextManager.getInstance().i18n.t('phovea:clue.provvis.logicActions')}" data-testid="${testId}-btn-vertical-logicActions">
    ...
    jp.find<HTMLElement>('.btn-collapse').on('click', (evt) => {
      ...
      $p.select('.btn-collapse').attr('title', collapsed ? I18nextManager.getInstance().i18n.t('phovea:clue.provvis.showProvenanceGraph') as string : I18nextManager.getInstance().i18n.t('phovea:clue.provvis.hideProvenanceGraph') as string);
  ...
  private onStateClick(d: StateRepr) {
    ...
    this.data.selectState(d.s, SelectionUtils.toSelectOperation(<MouseEvent>d3.event));
  ...
  update() {
    ...
    const $statesEnter = $states.enter().append('div')
      ...
      .on('dragenter', function () {
        if (DnDUtils.getInstance().hasDnDType(<DragEvent>d3.event, 'application/phovea-prov-state')) {
    ...
    const testId = 'provvis-state-actions';
    ...
    $toolbarEnter.append('i').attr('class', 'bookmark far fa-bookmark').attr('data-testid', `${testId}-bookmark`).on('click', function (d) {
    ...
    $toolbarEnter.append('i').attr('class', 'fas fa-edit').attr('data-testid', `${testId}-edit`).on('click', (d) => {
      const e = <Event>d3.event;
      d.showDialog();
      e.stopPropagation();
      e.preventDefault();
  ...
  static createLayoutedProvVis(data: ProvenanceGraph, parent: Element, options = {}) {
